...
void testHornetGamepad(s32 device_id, u8 endpoint, u8 bMaxPacketSize0)
{
	...
	// patterns overlap, therefore need exact match
	// which means that you cannot detect if two buttons are pressed at the same time (needs rework)
	...
	// example how to allow that two buttons are pressed at the same time
//	if (buf[5] & ((0x1F ^ 0x2F) & 0x1F))
//	{
//		printf("1=B");
//	}
//	if (buf[5] & ((0x1F ^ 0x2F) & 0x2F))
//	{
//		printf("2=A");
//	}
